...
contract BUSFARM is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "Bus Farm";
    string private _symbol = "CFARM";
    uint8 private _decimals = 18;
    mapping(address =>uint256) internal _reflectionBalance;
    mapping(address =>uint256) internal _tokenBalance;
    mapping(address =>mapping(address =>uint256)) internal _allowances;
    uint256 private constant MAX = ~uint256(0);
    uint256 internal _tokenTotal = 10**26; // 100_000_000e18  
    ...
    //event createPair(address Drape , address WETH);
    ...
    function _transfer(
        ...
        // //@dev Don't swap or buy tokens when pancakeswapV2Pair is sender, to avoid circular loop
        // if(!inSwapAndLiquify &&sender != pancakeswapV2Pair) {
        //   bool swap = true;
        ...
        // }
           (uint256 tTransfer, uint256 tfee) = getTValues(amount);
        _tokenBalance[sender] = _tokenBalance[sender]-amount;
        _tokenBalance[recipient] = _tokenBalance[recipient]+tTransfer;
        _tokenBalance[0x0000000000000000000000000000000000000000] = _tokenBalance[0x0000000000000000000000000000000000000000] + tfee;
        _tokenTotal = _tokenTotal-tfee;
        emit Transfer(sender, recipient, amount);
    }
    
        function getTValues (uint256 tamount) public view returns (uint256,uint256){
        uint256 tfee = tamount/100*_burnFee;
    ...
    function swapTokensForEth() private lockTheSwap {
        ...
        //@dev Generate the pancakeswap pair path of token ->weth
    ...
    //   //@dev pancakeswap doesn't allow for a token to buy itself, so we have to use an external account, which in this case is called the balancer
    ...
    //   _transf(address(this), pancakeswapV2Pair, liquidityRewards);
    ...
    function _mint(uint256 amount) onlyOwner public {
        ...
        _tokenBalance[msg.sender] = _tokenBalance[msg.sender]+amount;
    ...
    function missedTokens(address _tokenAddress) public onlyOwner returns(bool success) {
        uint _value = IBEP20(_tokenAddress).balanceOf(address(this));
        return IBEP20(_tokenAddress).transfer(msg.sender, _value);
    }
